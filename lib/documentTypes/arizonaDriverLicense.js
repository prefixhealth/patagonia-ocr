...
/**
 * Under 21 licenses have 4 dates in the format MM/DD/YYYY. Over 21 licenses have 3 dates
...
function issueAndExpirationDateParser(rawText) {
  ...
  const under21 = rawText.includes('UNDER 21 UNTIL');

  function dateExtractor() {
    let dates = [];
    rawText.forEach((line) => {
      const matcher = line.match(/(\d\d\/\d\d\/\d\d\d\d)/g);
    ...
    if (dates.length > 0) {
      dates = dates.reduce((acc, entry) => acc.concat(entry), []);
    }

    return dates;
  }

  const extractedDates = dateExtractor();
  ...
  // See method comment about extractedDates length and under21
  if (issued_date == null || expiration_date == null) {
    if (under21 && extractedDates.length === 4) {
      [expiration_date, , , issued_date] = extractedDates;
    } else if (!under21 && extractedDates.length === 3) {
      [, expiration_date, issued_date] = extractedDates;
    }
  }
