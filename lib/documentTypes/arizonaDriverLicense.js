...
module.exports = {
  ...
  extractor: (keyValues, rawText) => {
    ...
    let issued_date, expiration_date;
    let expAndIssLine = rawText.find((line) => line.match(/^4b(.*)$/g) || line.match(/^.+[E,e][X,x][P,p].+[I,i][S,s][S,s].+$/g));
    if(expAndIssLine) {
        let dates = expAndIssLine.match(/^.*(\d\d\/\d\d\/\d\d\d\d).*(\d\d\/\d\d\/\d\d\d\d).*$/);
