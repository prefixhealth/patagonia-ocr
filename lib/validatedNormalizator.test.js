...
const { normalizer: azDlNormalizer } = require('./documentTypes/arizonaDriverLicense');
...
it('Expect AZ diver license data to be normalized', async () => {
  const validated = {
    ...
    state: 'AZ',
    zip_code: '85007',
    issued_date: '04/01/2016',
    expiration_date: '04/01/2024'
  };

  const response = normalizeValidated(validated, azDlNormalizer);
  ...
  expect(response['Bill Amount']).toBe(undefined);
  expect(response['Bill Date']).toBe(undefined);
  expect(response['Account Number']).toBe(undefined);
  expect(response['Issued Date']).toBe('04-01-2016');
  expect(response['Expiration Date']).toBe('04-01-2024');
});
...
it('Expect SRP bill data to be normalized', async () => {
  ...
  expect(response['Account Number']).toBe('651-221-000');
  expect(response['Issued Date']).toBe(undefined);
  expect(response['Expiration Date']).toBe(undefined);
});
...
it('Expect APS bill data to be normalized', async () => {
  ...
  expect(response['Bill Date']).toBe('05-08-2020');
  expect(response['Status']).toBe('SUCCEEDED');
  expect(response['Account Number']).toBe('4563211000');
  expect(response['Issued Date']).toBe(undefined);
  expect(response['Expiration Date']).toBe(undefined);
});
...
it('Expect CITYOFPHOENIXWATER bill data to be normalized', async () => {
  ...
  expect(response['Errors']).toBe('street_address_line_2');
  expect(response['Status']).toBe('SUCCEEDED');
  expect(response['Account Number']).toBe('6372280000');
  expect(response['Issued Date']).toBe(undefined);
  expect(response['Expiration Date']).toBe(undefined);
});
...
it('Expect SOUTHWESTGAS bill data to be normalized', async () => {
  ...
  expect(response['Errors']).toBe('street_address_line_2');
  expect(response['Status']).toBe('SUCCEEDED');
  expect(response['Account Number']).toBe('421-6415508-002');
  expect(response['Issued Date']).toBe(undefined);
  expect(response['Expiration Date']).toBe(undefined);
});
