...
it('Expect AZ diver license to be detected and processed', async () => {
  const ocr = {
    ...
    rawText: [
      ...
      '123 MAIN ST',
      'PHOENIX, AZ 85007',
      '4b EXP 04/01/2024 4a ISS 04/01/2016',
      '15 SEX M',
      '18 EYES BRO VETERAN',
  ...
  expect(response.extracted.bill_amount).toBe(undefined);
  expect(response.extracted.bill_date).toBe(undefined);
  expect(response.extracted.account_number).toBe(undefined);
  expect(response.extracted.issued_date).toBe('04/01/2016');
  expect(response.extracted.expiration_date).toBe('04/01/2024');
});
...
it('Expect APS bill to be detected and processed', async () => {
  ...
  expect(response.extracted.street_address_line_2).toBe(undefined);
  expect(response.extracted.city).toBe('PHOENIX');
  expect(response.extracted.state).toBe('AZ');
  ...
  expect(response.extracted.issued_date).toBe(undefined);
  expect(response.extracted.expiration_date).toBe(undefined);
});
...
it('Expect CITYOFPHOENIXWATER bill to be detected and processed', async () => {
  ...
  expect(response.extracted.bill_date).toBe('5/28/2020');
